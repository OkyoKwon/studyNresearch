...
## 1-1. VASP 분류와 DeFi 쟁점

### VASP 정의

FATF는 **VASP(Virtual Asset Service Provider)**를 다음 활동을 수행하는 자연인 또는 법인으로 정의한다:

| 활동 유형 | 설명 | 예시 |
|----------|------|------|
| 가상자산-법정화폐 교환 | 매수/매도 | 거래소 원화 마켓 |
| 가상자산 간 교환 | 토큰 간 스왑 | 거래소 BTC/ETH 마켓 |
| 가상자산 이전 | 송금·출금 | 지갑 간 전송 서비스 |
| 가상자산 보관·관리 | 수탁 서비스 | 커스터디 지갑 |
| 가상자산 관련 금융 서비스 | 발행·참여 | ICO/STO 플랫폼 |

### DeFi 프로토콜의 VASP 해당 여부

DeFi 프로토콜이 VASP에 해당하는지는 **실질적 통제권(control or sufficient influence)**을 기준으로 판단한다.

```mermaid
graph TD
    PROTOCOL[DeFi 프로토콜] --> Q1{소유자/운영자가<br/>존재하는가?}
    Q1 -->|Yes| Q2{고객 대신<br/>가상자산을 통제하는가?}
    Q1 -->|No: 완전 탈중앙화| NOT_VASP[VASP 미해당]
    Q2 -->|Yes| VASP[VASP 해당<br/>AML/KYC 의무]
    Q2 -->|No: 비수탁| GRAY[회색 영역<br/>관할별 판단 상이]
```

| 서비스 유형 | VASP 해당 여부 | 근거 |
|-----------|-------------|------|
| 중앙화 거래소 (CEX) | 해당 | 자산 수탁 + 교환 |
| DEX (비수탁, 온체인) | 논쟁 중 | 스마트 컨트랙트가 실행, 운영자 통제 범위 불명확 |
| 렌딩 프로토콜 | 논쟁 중 | 비수탁이나 금융 서비스 제공 |
| 볼트 운영자 | 가능성 높음 | 전략 설계·실행에 운영자 관여 시 |
| 비수탁 지갑 | 미해당 | 사용자가 키를 직접 관리 |

### 비수탁 서비스의 VASP 우회 구조

"VASP 우회"란 규제 의무를 회피하기 위해 비수탁(non-custodial) 구조를 채택하는 것을 의미한다. 그러나 형식적으로 비수탁이더라도 실질적 통제권이 있으면 VASP로 분류될 수 있다.

| 관할 | VASP 범위 | DeFi 포함 여부 |
|------|---------|--------------|
| **한국** | 특금법상 VASP 신고제 | 현재 DEX 미포함, 향후 확대 논의 |
| **미국** | FinCEN MSB + SEC 증권법 | SEC는 DeFi에도 증권법 적용 입장 |
| **EU** | MiCA CASP + "완전 탈중앙화" 제외 | 제외 범위 해석 논쟁 |
| **싱가포르** | MAS 라이선스 | 실질적 통제 기준 적용 |

!!! danger "VASP 우회의 법적 리스크"
    형식적으로 비수탁 구조를 취하더라도, (1) 프론트엔드 운영, (2) 수수료 수취, (3) 전략 설계·변경 권한, (4) 관리자 키 보유 등이 확인되면 VASP로 재분류될 수 있다. 특히 증권사 등 기존 금융기관이 DeFi 서비스를 제공할 경우, 기존 금융업 인가와의 관계도 검토해야 한다.

...
## 볼트 전략 (Vault Strategy)

**볼트(Vault)**는 DeFi에서 자동화된 수익 전략을 실행하는 스마트 컨트랙트 컨테이너다. 사용자가 자산을 예치하면, 볼트가 사전 정의된 전략(자동 복리, 차익거래, 리밸런싱 등)을 자동 실행하여 수익을 극대화한다. Yearn Finance가 볼트 개념을 대중화했으며, Sommelier, Beefy Finance 등이 다양한 볼트 전략을 제공한다.

```mermaid
graph LR
    USER[사용자] -->|자산 예치| VAULT[볼트 컨트랙트]
    VAULT -->|전략 A| LEND[렌딩 프로토콜]
    VAULT -->|전략 B| LP[유동성 풀]
    VAULT -->|전략 C| STAKE[스테이킹]
    LEND -->|이자| VAULT
    LP -->|수수료| VAULT
    STAKE -->|보상| VAULT
    VAULT -->|자동 복리| VAULT
...
| 유형 | 설명 | 대표 사례 |
|------|------|----------|
| **단일 자산 볼트** | 하나의 자산으로 렌딩·스테이킹 수익 추구 | Yearn USDC vault |
| **LP 볼트** | 유동성 풀 수수료 + 보상 토큰 자동 복리 | Beefy Finance |
| **테마틱 볼트** | 특정 투자 테마(BTC+Gold, AI 토큰 등) 기반 복합 전략 | Sommelier, Supercycl |
| **인덱스 볼트** | 다수 자산을 비율 기반으로 자동 리밸런싱 | Balancer 관리형 풀 |
| **구조화 볼트** | 옵션 매도(covered call) 등 구조화 상품 전략 | Ribbon Finance, Thetanuts |

### 테마틱 볼트

**테마틱 볼트**는 특정 시장 테마나 투자 컨셉에 맞춰 복수 자산을 조합하고 수익 전략을 실행하는 볼트다. 예를 들어 "BTC + Gold" 테마 볼트는 비트코인과 금 토큰(PAXG)을 조합하여 인플레이션 헷지 전략을 자동화한다. 기관 투자자(증권사, 자산운용사)가 DeFi 수익 상품을 리테일에 제공하는 수단으로 주목받고 있다.

### Running Profit vs 일회성 이자 농사

| 구분 | Running Profit | 일회성 이자 농사 |
|------|---------------|----------------|
| 수익 모델 | 지속적 수수료·이자·스프레드 추출 | 프로토콜 토큰 보상 의존 |
| 지속 가능성 | 높음 (실제 거래 수요 기반) | 낮음 (인플레이션 의존) |
| 전략 복잡도 | 높음 (자동 리밸런싱, 리스크 관리) | 낮음 (예치 후 방치) |
| 대표 사례 | Sommelier, 기관 DeFi 볼트 | 초기 Yield Farming |

!!! warning "볼트 리스크"
    볼트는 자동화된 전략 실행의 편의성을 제공하지만, (1) 스마트 컨트랙트 취약점, (2) 전략 실패 시 원금 손실, (3) 하위 프로토콜 해킹에 의한 연쇄 손실, (4) 청산 연쇄(liquidation cascade) 리스크가 존재한다. 기관이 볼트 상품을 리테일에 제공할 경우 투자자 보호와 [증권성 판단](../sto/concepts.md) 이슈가 추가된다.

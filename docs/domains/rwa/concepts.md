...
## 원자재 토큰화 (Commodity Tokenization)

**원자재 토큰화**는 금, 은, 원유, 탄소크레딧 등 물리적 원자재의 소유권을 블록체인 토큰으로 변환하는 것이다. 특히 **금 토큰화**가 가장 성숙한 시장으로, PAXG(Paxos Gold)가 대표적이다.

### 토큰화 금 시장

| 항목 | PAXG (Paxos Gold) | XAUT (Tether Gold) | DGX (Digix) |
|------|-------------------|-------------------|-------------|
| 발행사 | Paxos Trust Company | Tether (TG Commodities) | Digix Global |
| 규제 | NYDFS 규제 신탁회사 | 비규제 | 비규제 |
| 담보 | LBMA 인증 금 1:1 | LBMA 인증 금 1:1 | 금 1g = 1 DGX |
| 보관 | Brink's 런던 금고 | 스위스 금고 | 싱가포르 금고 |
| 실물 인출 | 가능 (최소 1온스) | 가능 (최소 430온스) | 가능 |
| 블록체인 | Ethereum | Ethereum, Tron | Ethereum |
| 시가총액 | ~$500M+ | ~$700M+ | 서비스 축소 |
| 감사 | 월간 제3자 감사 | 분기 감사 | 분기 감사 |

### 물리적 수탁 특이사항

원자재 토큰화에서 물리적 수탁은 일반 금융 자산보다 복잡하다:

- **순도 인증**: LBMA(London Bullion Market Association) Good Delivery 기준 충족 필수
- **보관 보험**: 분실·도난·자연재해에 대한 보험 가입
- **감사 체계**: 독립 제3자 감사인이 실물 보유량과 토큰 발행량 일치 검증
- **보관 비용**: 연 0.02~0.03% 수준, 토큰 보유 수수료로 전가

### DeFi 활용

토큰화 금은 DeFi에서 다양하게 활용된다:

- **담보 대출**: PAXG를 [Aave](../defi/products/aave.md), [MakerDAO](../defi/products/makerdao.md) 등에 담보로 예치하여 스테이블코인 대출
- **유동성 풀**: PAXG/USDC, PAXG/ETH 등 LP 구성
- **테마틱 볼트**: BTC+Gold 등 인플레이션 헷지 [볼트 전략](../defi/concepts.md) 구성 가능

!!! info "한국에서 금토큰 발행 시 고려사항"
    한국에서 PAXG와 같은 금토큰을 발행하려면 (1) VASP 등록 또는 금융업 인가 필요, (2) 실물 금 수탁 인프라 구축, (3) 상품의 [증권성 판단](../sto/concepts.md) 검토(수익 분배 구조에 따라 투자계약증권 해당 가능), (4) [KYC/AML](../aml-kyc/concepts.md) 체계 구축이 필요하다. 관할별 원자재 규제(상품거래법 등)와 가상자산 규제의 중첩도 검토해야 한다.

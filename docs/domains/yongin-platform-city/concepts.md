...
!!! info "세종 스마트시티 벤치마크"
    세종 5-1생활권 스마트시티는 국내 최초로 도시 데이터 플랫폼을 실증 중이다. 교통·환경·에너지 데이터를 통합 관리하며, 자율주행 셔틀을 시범 운행하고 있다. 용인플랫폼시티는 세종의 실증 결과를 바탕으로 더 큰 규모(약 4.6배)에서 기술을 적용하게 된다.
...
!!! warning "클러스터와 플랫폼시티의 거리"
    SK하이닉스 클러스터(원삼면)와 플랫폼시티(남사·이동읍)는 약 10~15km 거리로, 직접 인접한 것은 아니다. 자율주행 셔틀·BRT 등 도시 내부 교통 연결이 실제로 구현되어야 "배후 자족도시"로서의 가치가 실현된다.
